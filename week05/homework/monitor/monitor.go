...
type httpRequest struct {
	ctx     context.Context   //用于支持超时和取消
	method  string            //GET、POST等HTTP方法
	url     string            //请求URL，例如http://example.com/api
	headers map[string]string //请求头部字段，例如{"User-Agent": "Monitor/1.0"}
}
...
func NewMonitor(timeout time.Duration, verbose bool, proxy string) *Monitor {
	return &Monitor{
		timeout:    timeout,
		verbose:    verbose,
		logger:     log.New(os.Stdout, "[verbose] ", log.LstdFlags),
		taskDelay:  artificialTaskDelay,
		httpClient: newHTTPClient(timeout, proxy),
	}
}
