...
这显然是不对的，因为比如李老师上了id为2的课程，但是course_ids字段中可能包含2、或者12的课程，最终查询结果如下：
![未更改prompt前.png](./原始查询结果.png)
我们结合数据库实际的内容，发现多查询了
![多查询的学生.png](./多查询的学生.png)
因此后续更改prompt，精细化的说明了students这个表中的重要字段course_ids，并使用one-shot的提示词编写方法给出了示例，增加的prompt内容如下：
```txt
